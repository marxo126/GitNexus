...
withTestLbugDB('wcag-audit-e2e', (handle) => {
  ...
  describe('compliance score', () => {
    it('returns a compliance score with percent, met, and total', async () => {
      ...
      // We have 3 criteria: 1.1.1 (has violation+pass), 1.3.1 (has violation+pass), 2.1.1 (warning only), 2.4.2 (violation only)
      // Criteria with 0 violations: 2.1.1 (only warning) => met=1, total depends on filter
      expect(result.score.total).toBeGreaterThanOrEqual(3);
      expect(result.score.percent).toBeLessThan(100); // some violations exist
    });

    it('returns correct violation and warning counts', async () => {
      const result = await backend.callTool('wcag_audit', {});

      expect(result.violations).toBeGreaterThanOrEqual(3); // img-alt, input-label, page-titled
      expect(result.warnings).toBeGreaterThanOrEqual(1); // keyboard
      expect(result.total).toBe(result.violations + result.warnings);
    });
  });
