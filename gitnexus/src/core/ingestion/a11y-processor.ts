...
function findSourceNodeId(signal: A11ySignal, graph: KnowledgeGraph): string | undefined {
  ...
  for (const prefix of ['Function', 'Method']) {
    ...
    const candidateId = `${prefix}:${signal.filePath}:${(signal as any).enclosingFunction || ''}`;
    if ((signal as any).enclosingFunction && graph.getNode(candidateId)) {
      return candidateId;
    }
  }
...
export function processA11ySignals(
  ...
  for (const signal of signals) {
    ...
    else if (signal.status === 'warning') warnings++;
  ...
  return { signalsCreated, edgesCreated, violations, warnings };
}
