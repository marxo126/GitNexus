...
import { extractGuardClauses, extractGuardedCalls } from './guard-extractor.js';
import { generateId } from '../../lib/utils.js';
import type { ExtractedFetchCall, ExtractedRoute, ExtractedDecoratorRoute, ExtractedToolDef, ExtractedParameter } from './workers/parse-worker.js';
...
async function runChunkedParseAndResolve(
  ...
  // Accumulate function/method parameters for data flow tracking
  ...
  return { exportedTypeMap, allFetchCalls, allExtractedRoutes, allDecoratorRoutes, allToolDefs, allParameters };
}
...
export const runPipelineFromRepo = async (
  ...
  try {
    ...
    const { exportedTypeMap, allFetchCalls, allExtractedRoutes, allDecoratorRoutes, allToolDefs, allParameters } = await runChunkedParseAndResolve(
    ...
    {
      const functionNodes = [...graph.iterNodes()].filter(n =>
        n.label === 'Function' || n.label === 'Method'
      );

      if (functionNodes.length > 0) {
        const fileGroups = new Map<string, typeof functionNodes>();
        ...
        for (const [filePath, content] of fileContents) {
          ...
          const guards = extractGuardClauses(content, language);
          if (guards.length > 0) {
            const nodes = fileGroups.get(filePath) || [];
            for (const node of nodes) {
              const nodeGuards = guards.filter(g => g.functionName === node.properties.name);
          ...
          const guardedCalls = extractGuardedCalls(content, language);
          for (const gc of guardedCalls) {
            for (const rel of graph.iterRelationships()) {
              if (rel.type !== 'CALLS') continue;
              const targetNode = graph.getNode(rel.targetId);
              if (!targetNode || targetNode.properties.name !== gc.calledName) continue;
              const sourceNode = graph.getNode(rel.sourceId);
              if (!sourceNode || sourceNode.properties.filePath !== filePath) continue;
              if (gc.functionName && sourceNode.properties.name !== gc.functionName) continue;
