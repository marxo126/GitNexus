...
export function parseShapeReason(reason: string | null | undefined): {
  ...
  const confMatch = reason.match(/^shape-(\d*\.?\d+)/);
...
export class LocalBackend {
  ...
  private async dataFlow(repo: RepoHandle, params: { query?: string; slotKind?: string; mismatchesOnly?: boolean }): Promise<any> {
    await this.ensureInitialized(repo.id);
    ...
    const slotRows = await executeParameterized(repo.id, `
      MATCH (n:StateSlot)
      WHERE n.id STARTS WITH 'StateSlot:' ${slotFilter}
    ...
    // 4. Filter if mismatchesOnly
    const filtered = params.mismatchesOnly
