...
export class LocalBackend {
  ...
  private async context(repo: RepoHandle, params: {
    ...
    try {
      producesRows = await executeParameterized(repo.id, `
      ...
      `, { symId });
    } catch (e) { logQueryError('context:produces-slots', e); }
    try {
      consumesRows = await executeParameterized(repo.id, `
        MATCH (n {id: $symId})-[r:CodeRelation]->(s:StateSlot)
        WHERE r.type = 'CONSUMES'
        RETURN s.name AS slotName, s.slotKind AS slotKind, s.cacheKey AS cacheKey, r.reason AS reason
      `, { symId });
    } catch (e) { logQueryError('context:consumes-slots', e); }
  ...
  private async _impactImpl(repo: RepoHandle, params: {
    ...
    let stateSlotImpact: Array<{ slotName: string; slotKind: string; consumers: Array<{ name: string; filePath: string; accessedKeys?: string[] }> }> = [];
    try {
      ...
      if (impactedNames.length > 0) {
        const nameList = impactedNames.map(n => `'${String(n).replace(/'/g, "''")}'`).join(', ');
        // Find StateSlots that impacted functions PRODUCE to
        const producedSlotRows = await executeQuery(repo.id, `
        ...
        if (producedSlotRows.length > 0) {
          const slotIds = producedSlotRows.map((r: any) => `'${String(r.slotId ?? r[0]).replace(/'/g, "''")}'`).join(', ');
          // Find consumers of those StateSlots
          const consumerRows = await executeQuery(repo.id, `
            MATCH (fn)-[r:CodeRelation]->(s:StateSlot)
          ...
          stateSlotImpact = producedSlotRows.map((r: any) => {
    ...
    } catch (e) { logQueryError('impact:state-slot-traversal', e); }
